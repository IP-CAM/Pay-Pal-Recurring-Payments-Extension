...
class ModelSaleRecurringOrder extends Model {
	public function getReasons() {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "paypal_recurring_reasons ORDER BY id ASC");
		
		return $query->rows;
		
		
		}
		public function getTotalGross($pid) {
      	$query = $this->db->query("SELECT SUM(paypal_recurring_lastpaymentamount) AS total FROM `" . DB_PREFIX . "order` WHERE paypal_recurringprofile_id = '" . (string)$pid . "'");
	...
	public function deleteOrder($order_id) {
        ...
        $this->db->query("DELETE FROM `" . DB_PREFIX . "order` WHERE paypal_recurringprofile_id = '" . (string)$order_query->row['paypal_recurringprofile_id'] . "'");
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id) AS customer FROM `" . DB_PREFIX . "order` o WHERE order_id = '" . (int)$order_id . "'");
		if ($order_query->num_rows) {
			$reward = 0;
			...
			return array(
				...
				'recurring_check'                => $order_query->row['paypal_recurringprofile_id'],
			'paypal_transaction_id'                => $order_query->row['paypal_transaction_id'],
			'paypal_recurring_status'                => $order_query->row['paypal_recurring_status'],
			'paypal_recurring_desc'                => stripslashes($order_query->row['paypal_recurring_desc']),
			'paypal_recurring_autobillamount'                => $order_query->row['paypal_recurring_autobillamount'],
			'paypal_recurring_maxfailedpayments'                => $order_query->row['paypal_recurring_maxfailedpayments'],
			'paypal_recurring_lastpaymentamount'                => $order_query->row['paypal_recurring_lastpaymentamount'],
			'paypal_recurring_finalpaymentduedate'                => $order_query->row['paypal_recurring_finalpaymentduedate'],
			'paypal_recurring_subscribername'                => $order_query->row['paypal_recurring_subscribername'],
			'paypal_recurring_startdate'                => $order_query->row['paypal_recurring_startdate'],
			'paypal_recurring_reference'                => $order_query->row['paypal_recurring_reference'],
			'paypal_recurring_billingperiod'                => $order_query->row['paypal_recurring_billingperiod'],
			'paypal_recurring_regularbillingperiod'                => $order_query->row['paypal_recurring_regularbillingperiod'],
			'paypal_recurring_billingfrequency'                => $order_query->row['paypal_recurring_billingfrequency'],
			'paypal_recurring_regularbillingfrequency'                => $order_query->row['paypal_recurring_regularbillingfrequency'],
			'paypal_recurring_totalbillingcycles'                => $order_query->row['paypal_recurring_totalbillingcycles'],
			'paypal_recurring_regulartotalbillingcycles'                => $order_query->row['paypal_recurring_regulartotalbillingcycles'],
			'paypal_recurring_amount'                => $order_query->row['paypal_recurring_amount'],
			'paypal_recurring_regularamount'                => $order_query->row['paypal_recurring_regularamount'],
			'paypal_recurring_shippingamount'                => $order_query->row['paypal_recurring_shippingamount'],
			'paypal_is_original_order'                                      => $order_query->row['paypal_is_original_order'],
			'paypal_recurring_regularshippingamount'                => $order_query->row['paypal_recurring_regularshippingamount'],
			'paypal_recurring_taxamount'                => $order_query->row['paypal_recurring_taxamount'],
			'paypal_recurring_regulartaxamount'                => $order_query->row['paypal_recurring_regulartaxamount'], 
			'paypal_recurring_nextbilldate'                => $order_query->row['paypal_recurring_nextbilldate'], 
			'paypal_recurring_cyclescompleted'                => $order_query->row['paypal_recurring_cyclescompleted'], 
			'paypal_recurring_cyclesremaining'                => $order_query->row['paypal_recurring_cyclesremaining'], 
			'paypal_recurring_outstandingbalance'                => $order_query->row['paypal_recurring_outstandingbalance'], 
			'paypal_recurring_failedpaymentcount'                => $order_query->row['paypal_recurring_failedpaymentcount'], 
			'paypal_recurring_lastpaymentdate'                => $order_query->row['paypal_recurring_lastpaymentdate'], 
			'paypal_recurring_lastpaymentamount'                => $order_query->row['paypal_recurring_lastpaymentamount'], 
			'paypal_recurring_cctype'                => $order_query->row['paypal_recurring_cctype'], 
			'paypal_recurring_ccnumber'                => $order_query->row['paypal_recurring_ccnumber'], 
			'paypal_recurring_ccexpire'                => $order_query->row['paypal_recurring_ccexpire'], 
			'paypal_recurring_maestrostartdate'                => $order_query->row['paypal_recurring_maestrostartdate'], 
			'paypal_recurring_maestronumber'                => $order_query->row['paypal_recurring_maestronumber'], 
			'paypal_recurring_cancelreason'                => $order_query->row['paypal_recurring_cancelreason'], 
			'paypal_recurring_trial_status'                => $order_query->row['paypal_recurring_trial_status'], 		
	...
	public function getParentTotal($profile_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE paypal_recurringprofile_id = '" . (string)$profile_id . "' AND paypal_is_original_order = '0'");
         
		return $query->row['total'];
	}
	public function getParentOrders($pid) {
		$sql = "SELECT o.order_id,o.paypal_recurring_lastpaymentamount,o.paypal_recurringprofile_id,o.paypal_transaction_id,o.email, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o ";
		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			$sql .= " WHERE o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			$sql .= " WHERE o.order_status_id > '0'";
		}
		$sql .= " AND o.paypal_recurringprofile_id = '" . (string)$pid . "'";
		   $sql .= " AND o.paypal_is_original_order = '0'";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id,o.paypal_recurring_lastpaymentamount,o.paypal_recurringprofile_id,o.paypal_recurring_trial_status, o.paypal_recurring_count, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status,  o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o ";
		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			$sql .= " WHERE o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			$sql .= " WHERE o.order_status_id > '0'";
		...
		if (!empty($data['filter_order_id'])) {
          ...
          if (!empty($data['filter_profile_id'])) {
			$sql .= " AND o.paypal_recurringprofile_id = '" . (string)$data['filter_profile_id'] . "'";
		}
		...
		$sql .= " AND o.paypal_is_original_order = '1'";
		
        
		$sort_data = array(
			'o.order_id',
			'o.paypal_recurringprofile_id',
			'customer',
			'status',
			'o.date_added',
			'o.date_modified',
	...
	public function getOrderProducts($order_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` op LEFT JOIN `" . DB_PREFIX . "order` o ON op.order_id = o.order_id WHERE op.order_id = '" . (int)$order_id . "'");
