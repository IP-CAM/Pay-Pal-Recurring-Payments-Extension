...
//==============================================================================
class ControllerCheckoutRecurringFailureContinue extends Controller { 
	public function index() { 	
		...
		$this->language->load('checkout/recurring_failure_continue');
		...
		if ($this->customer->isLogged()) {
			...
			if($this->config->get('screen_confirm2')){
			$this->data['text_custom_message'] = html_entity_decode($this->config->get('screen_confirm2'), ENT_QUOTES, 'UTF-8');
			}else{
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/recurring_failure_continue.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/common/recurring_failure_continue.tpl';
		} else {
			$this->template = 'default/template/common/recurring_failure_continue.tpl';
		}
