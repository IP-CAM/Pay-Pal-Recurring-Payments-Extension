...
//==============================================================================
class ControllerCheckoutRecurringFailureCancel extends Controller { 
	public function index() { 	
		...
		$this->language->load('checkout/recurring_failure_cancel');
		...
		if ($this->customer->isLogged()) {
			...
			if($this->config->get('screen_confirm3')){
			$this->data['text_custom_message'] = html_entity_decode($this->config->get('screen_confirm3'), ENT_QUOTES, 'UTF-8');
			}else{
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/recurring_failure_cancel.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/common/recurring_failure_cancel.tpl';
		} else {
			$this->template = 'default/template/common/recurring_failure_cancel.tpl';
		}
